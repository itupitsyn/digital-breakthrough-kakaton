...
namespace RecSysConverter.TrainSet
{
    internal class TrainSetBuilder
    {
        public static void Build()
        {
            ...
            using (var processor = new BatchProcessor<DataSet>(5000, records => _ds.Append(records)))
            {
                bool skip = true;
                foreach (var entry in preset_dictionary)
                {
                    ...
                    if (candidates.Count >= 3 && neg_candidates.Count == 0)
                    {
                        ...
                        foreach (var candidate in candidates)
                        {
                            ...
                            var negative = totalReview > 0 ? candidate.dislikes / totalReview : 0.5;
                            if (max_neg < negative)
                            {
                                max_neg = negative;
                    ...
                    else if (candidates.Count >= 3 && pos_candidates.Count == 0)
                    {
                        ...
                        {
                            var totalReview = candidate.likes + candidate.dislikes;
                            var positive = totalReview > 0 ? candidate.likes / totalReview : 0.5;
                            if (max_pos < positive)
                            {
                                max_pos = positive;
